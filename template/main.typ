...
#show: project.with(
  ...
  contacts: 
    (
      "+86 183 2495 1966",
       link("mailto:princesszhi@foxmail.com", "princesszhi@foxmail.com"),  
...
#resume-section[技术能力]
- *语言*: 常用 C(C11), C++(C++11)Rust, ；了解 Go, Java, Python。
...
- *独立课题《证券竞价交易模拟撮合系统》的后端开发。*通过对 Pistache 框架（一种现代 C++17 编写的高效 REST 框架，异步非阻塞设计）、`rapidjson` 高性能序列化/反序列化库和 PostgreSQL 数据库（基于 SOCI 数据库访问层）的应用，实现了一个兼具正确性、公平性、并发性（异步 IO）的模拟撮合系统。
]
...
#resume-project(
  ...
  - 使用 C\# 语言开发。通过面向对象的底层程序设计，使得用户可以通过统一化菜单交互方式，对整个仿真过程进行深度自定义。
...
#resume-project(
  ...
  - 使用现代 C++17 开发，基于 Oat++ 框架(高性能、模块化的 C++ Web 框架)开发后端，支持跨平台和轻量级部署。
  - 使用 MongoDB （使用 MongoDB C++11 Driver）和 PostgreSQL （使用 Oat++ ORM）双数据库进行数据存储，平衡了安全性和灵活性的开发需求。
]
...
#resume-project(
  ...
  - 项目编程过程中利用 C++ 模板元编程机制进行简单的 DSL 生成，利用 TensorFlow C++ 进行 BERT 推理，利用 `owlcpp` 与 `fact++` 库实现 OWL 导出与推理。
]
...
#resume-project(
  ...
  - 深度学习模型的训练与测试使用 C++ 完成，基于 `libtorch` 实现，中文医学知识图谱构建系统的后端部分使用 Pistache 框架实现，。
]
